...
const LOCAL_NAME_KEY = "suiviLeoName";

function getLocalName(){
  try{
    const value = localStorage.getItem(LOCAL_NAME_KEY);
    return value ? value.trim().replace(/\s+/g," ").slice(0,30) : "";
  }catch(e){
    return "";
  }
}
...
function saveLocalName(name){
  try{ localStorage.setItem(LOCAL_NAME_KEY, name); }catch(e){}
}
...
async function ensureAuth(){
  // On force explicitement la persistance locale : le même navigateur doit garder
  // le même compte anonyme après fermeture/réouverture du site.
...
async function ensureProfile(user){
  ...
  // Sécurité anti-redemande : si Firebase a perdu/recréé l'identifiant anonyme
  // mais que le navigateur possède encore le prénom enregistré localement,
  // on recrée silencieusement le profil au lieu de poser la question à nouveau.
